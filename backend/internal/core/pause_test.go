...
func TestPauseController(t *testing.T) {
	t.Run("一時停止中は WaitIfPaused がブロックし、Resume で再開する", func(t *testing.T) {
	...
	t.Run("一時停止中に context キャンセルで WaitIfPaused が終了する", func(t *testing.T) {
		p := core.NewPauseController()
...
// TestCrawlerPause はクローラー実行中の一時停止が新規フェッチを抑止することを検証する。
func TestCrawlerPause(t *testing.T) {
	t.Run("一時停止中は新規フェッチが開始されない", func(t *testing.T) {
