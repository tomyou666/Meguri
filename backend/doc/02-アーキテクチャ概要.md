...
- `cmd/meguri/main.go` のみが具体プラグインを `_` インポートし、ビルドに組み込む。
- `internal/` 配下を Go の `internal` 仕様で外部利用から遮断する（このリポジトリ専用）。
- 将来の Fyne GUI 化は `internal/presentation/gui/` を追加し、`cmd/meguri-gui/main.go` を別途用意することで、CLI 版とユースケース層以下を完全に共有できる。
...
- 当面の動作確認は `cmd/meguri/main.go` から CLI（`internal/presentation/cli`）を呼び出す形にする。
...
- Fyne への移行手順の想定:
    ...
    2. `cmd/meguri-gui/main.go` を追加し、CLI と同じ `app.Initialize` を bind する。
    3. CLI 版バイナリは引き続き `cmd/meguri` から生成。
...
- ボット対策（UA / Cookie / chromium 起動フラグ）は `plugins.stealth` に集約する（`fetcher` 種別ごとに http / chromium を切り替え）。
...
- `cmd/meguri/main.go` のみが具体プラグインを副作用 import している。
