...
- 選択: `plugins.fetcher`（既定 `http`）。`Kernel.Init` で P3 プラグインを初期化済み。
    - `http`: `plugins/fetcher-http`（net/http 実装）
    - `chromium`: `plugins/fetcher-chromium`（chromedp 実装）
- 専用設定: `plugins.fetcher_config`（chromium 時の待機・ブラウザパス）、`plugins.stealth`（fetcher 種別ごとのボット対策）
    - `fetcher_config.browser_path`: ブラウザ実行ファイルの明示指定（空なら自動検出）。
    - `wait_until` / `wait_timeout` / `wait_visible_selector` / `network_idle_duration` / `network_idle_request_max_age`: Navigate 後のページ読み込み待機（`wait_until` 既定 `load`）。
    - `stealth.chromium`: UA / headless / hide_automation / user_data_dir / lang / window-size / Accept-Language 等。
    - `stealth.http`: User-Agent / Accept-Language / Cookie（`preprocessor-header` が転写）。
...
- 結果: `*model.Response`（ステータス、ヘッダ、Content-Type、Body、`FetchedAt(time.Time)`）。
    - `chromium` 時の既定は HTML レンダリング（`StatusCode` は成功時 `200` 固定、`Content-Type` は `text/html`）。
    - **例外**: `pdf.enabled=true` かつ URL パスが `.pdf` で終わる場合、CDP Fetch ドメイン（`RequestStageResponse` インターセプト + `fetch.GetResponseBody`）で `application/pdf` バイナリを返す（PDF ビューア HTML ではなく生バイナリ）。
- robots.txt 取得も **ページ取得と同じ P3 Fetcher** を使用する（`robots.Cache` が `Kernel.Fetcher().Get` を呼ぶ）。
- **取得並列上限**: `Kernel.Init` 後の `Fetcher` は `limitingFetcher` デコレータで包まれる場合がある。`Get` 内で `crawl.fetch_limits` に基づくセマフォを acquire してから inner プラグインを呼ぶ。Pipeline.Run は変更不要。
    - `http` / `chromium` それぞれ独立した上限（既定 16 / 2）。
    - `auto_calibrate` で起動時に Chromium 上限をメモリ計測で調整。`dynamic_chromium` で実行中に水位連動調整。
    - robots.txt 取得も同一 `Fetcher` 経由のため上限対象。
- エラーポリシー: 規定回数失敗 → 当該URL失敗。ログに最終エラーを記録。

**将来タスク（ブラウザプール）**: Chromium の `NewExecAllocator` 毎回起動をプール再利用に置き換える際は、`chromium_max_inflight` をプールサイズと揃える。
...
type Crawler struct {
    ...
    // fetcher は Kernel 経由（plugin.Fetcher）
    pipeline *Pipeline
...
- パスパターン:
    - `include_hosts` 指定時、`url.Host` が一致しない URL は除外。
    - `exclude_hosts` 指定時、`url.Host` が一致する URL は除外。
...
- 評価対象 User-Agent: `plugins.stealth.http.user_agent` の実効値（未設定なら `meguri/0.1`、robots 評価で未設定扱いは `*`）。
