...
```bash
./bin/meguri-cli --url https://example.com/ --stdout
```

### 単一 URL をファイルに保存

```bash
./bin/meguri-cli --url https://example.com/ --output-dir ./out
```
...
./bin/meguri-cli --config my-config.yaml --stdout
```
...
```bash
export MEGURI_CHROMIUM_PATH=/usr/bin/chromium
./bin/meguri-cli --url https://example.com/ --fetcher chromium --stdout
```
...
プラグインの追加・差し替えは [`cmd/meguri-cli/main.go`](cmd/meguri-cli/main.go) の副作用 import を編集して再ビルドします。
...
```text
cmd/meguri-cli/          # CLI エントリ（プラグインの副作用 import）
internal/
