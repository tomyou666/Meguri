...
- 既存ユーザー更新: Wails updater が実行中 exe の arch（`runtime.GOARCH`）に応じ `meguri-windows-amd64.zip` または `meguri-windows-arm64.zip` を取得し、**ユーザーがネイティブダイアログで同意した後**にダウンロード → exe 差し替え → 再起動
...
- [ ] 検知後、ネイティブダイアログに版番号と GitHub リリース URL が表示される
...
Wails updater はインストーラーではなく zip のみ参照する。実行中 exe のビルド arch（`runtime.GOARCH`）で `meguri-windows-{arch}.zip` を選択する。Wails 標準の更新ウィンドウは使わず（`WindowNone`）、Meguri 独自 UI + ネイティブ `MessageDialog` で確認する。
...
| [`front/internal/usecase/wails_service/update_dialog.go`](../front/internal/usecase/wails_service/update_dialog.go) | 差し替え前のネイティブ確認ダイアログ |
...
3. **確認** — ネイティブダイアログ（版・リリース URL テキスト、「更新して再起動」「リリースを開く」「後で」）
...
**更新:** 起動時に自動確認（更新があれば通知 toast。詳細からネイティブダイアログ）。手動は設定メニュー「更新を確認…」。同意後にダウンロードし、再起動で適用。リリース内容はダイアログ内の URL または「リリースを開く」から GitHub Release へ。
