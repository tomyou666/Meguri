...
- 既存ユーザー更新: Wails updater が実行中 exe の arch（`runtime.GOARCH`）に応じ `meguri-windows-amd64.zip` または `meguri-windows-arm64.zip` を取得し、**ユーザーが更新確認ウィンドウで同意した後**にダウンロード → exe 差し替え → 再起動
- インストール先: user スコープ（`%LOCALAPPDATA%\Programs\Meguri\`、管理者権限不要）。updater が差し替えるのもこの `meguri.exe`
...
セキュリティ修正は **必ず新しいバージョン番号**で出す。起動時 toast の「今後表示しない」は **その版だけ**非表示（`localStorage`）であり、次の版が出れば再通知される。
...
1. 版を決める（`X.Y.Z`）
2. [`CHANGELOG.md`](../CHANGELOG.md) を更新する（`## [Unreleased]` に追記し、リリース確定時は `## [X.Y.Z] - YYYY-MM-DD` として確定）
3. `go run ./tools/version-mng <X.Y.Z>`
4. フィードバックボタンを有効にする場合は GitHub Secret `VITE_FEEDBACK_URL` を登録（ローカルでは `front/frontend/.env` に同値を設定）
5. 変更をコミット（`CHANGELOG.md`・`version-nsis.nsh` 含む）
6. タグを付けて push:
...
7. [Actions](https://github.com/tomyou666/Meguri/actions) で `Release Windows` を確認
8. GitHub Release の資産・リリースノートを確認

### リリースノート（CHANGELOG）

CI は tag の版（`v` なし `X.Y.Z`）に対応する [`CHANGELOG.md`](../CHANGELOG.md) の節を [`changelog-extract`](../tools/changelog-extract/main.go) で抽出し、GitHub Release 本文に載せる。節が無い・空の場合は従来どおり GitHub の自動生成ノート（`generate_release_notes`）にフォールバックする。

ローカル確認:

```bash
go run ./tools/changelog-extract 1.0.0
```
...
- [ ] 検知後、別 Webview ウィンドウに版番号と GitHub リリース URL が表示される
- [ ] 「更新して再起動」でダウンロード後にアプリが再起動し、版が上がる
- [ ] 「リリースを開く」で既定ブラウザ（WebView 外）に GitHub Release が開く
- [ ] 起動時 toast・設定メニューの更新バッジが期待どおり動く
- [ ] `SHA256SUMS` に zip ファイル名が実ファイルと完全一致している
- [ ] セキュリティ修正なら版を上げた（旧版向け toast 非表示だけでは届かない）

### 自動更新について

Wails updater はインストーラーではなく zip のみ参照する。実行中 exe のビルド arch（`runtime.GOARCH`）で `meguri-windows-{arch}.zip` を選択する。Wails 標準の更新ウィンドウは使わず（`WindowNone`）、Meguri 独自の別 Webview ウィンドウ（`/?view=update-prompt`）で確認する。

実装の中心:

| 箇所 | 役割 |
|------|------|
| [`front/main.go`](../front/main.go) | updater 初期化、起動時 `CheckOnStartup`、6 時間ごとの `Check` のみ |
| [`front/internal/usecase/wails_service/update_service.go`](../front/internal/usecase/wails_service/update_service.go) | `Check` / `PromptUpdate` / `ApplyUpdate` 等の RPC |
| [`front/internal/usecase/wails_service/update_window.go`](../front/internal/usecase/wails_service/update_window.go) | 差し替え前の更新確認 Webview ウィンドウ |
| [`front/frontend/src/components/update/UpdatePromptApp.tsx`](../front/frontend/src/components/update/UpdatePromptApp.tsx) | 更新確認 UI（3 ボタン） |
| フロント [`useUpdateAvailability`](../front/frontend/src/hooks/useUpdateAvailability.ts) 等 | 起動 toast・バッジ・リリース URL を既定ブラウザで開く |

#### ユーザー側の流れ

1. **検出** — 起動時・6 時間ごと・手動「更新を確認…」で GitHub Releases を `Check`（この時点ではダウンロードしない）
2. **通知** — 更新ありなら起動 toast（「今後表示しない」可）と設定メニューのバッジ
3. **確認** — 別 Webview ウィンドウ（版・リリース URL、「更新して再起動」「リリースを開く」「後で」）
4. **適用** — 「更新して再起動」後に zip を取得・検証し、`%TEMP%\wails-update-*\` にステージ → helper が `%LOCALAPPDATA%\Programs\Meguri\meguri.exe` を差し替え → 再起動

リリースページは WebView 内ではなく [`openExternalBrowserUrl`](../front/frontend/src/lib/externalLinkDelegation.ts)（`Browser.OpenURL`）経由で既定ブラウザを開く。OS ネイティブメニュー「更新を確認…」から開く場合のみ Go 側 `app.Browser.OpenURL` を使う。
...
#### dev ビルドで updater を試す場合

`-tags dev` かつ `main.currentVersion=dev` だと GitHub 上の最新版が常に「新しい」と判定される。exe 差し替えの E2E では、破壊を避けるため差し替え先を明示する:

```powershell
$env:MEGURI_UPDATER_SWAP_TARGET = "C:\path\to\copy\meguri-test.exe"
```

`MEGURI_UPDATER_SWAP_TARGET` は [`.env.example`](../.env.example) を `copy .env.example .env` して値を入れる。**dev ビルド（`-tags dev`）は起動時にリポジトリルートの `.env` を Go が読み込む。** VS Code の **Wails3: Dev + TSX** 系でも同じ `.env` が使える。

未設定時は実行中 exe（`os.Executable()`）が差し替え対象。helper ログは `%TEMP%\wails-update-<pid>.log`。
...
**更新:** 起動時に自動確認（更新があれば通知 toast。詳細から更新確認ウィンドウ）。手動は設定メニュー「更新を確認…」。同意後にダウンロードし、再起動で適用。リリース内容はウィンドウ内の URL または「リリースを開く」から GitHub Release へ。
