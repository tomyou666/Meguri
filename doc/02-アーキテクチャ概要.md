...
- プレゼンテーション層（CLI / Fyne）は `usecase` を呼び出すだけで、内部実装に踏み込まない。
...
flowchart TB
    ...
    subgraph core [Core / Microkernel]
        ...
        Host[Host services: logger config bus]
    end
...
│       ├── httpclient/             # net/http ラッパー
│       │   └── client.go
│       ├── storage/                # ファイル保存
│       │   └── file.go
│       └── robots/                 # robots.txt 取得・判定
│           └── robots.go
├── plugins/                        # 具体プラグイン実装
...
- `cmd/goscraper/main.go` のみが具体プラグインを `_` インポートし、ビルドに組み込む。
- `internal/` 配下を Go の `internal` 仕様で外部利用から遮断する（このリポジトリ専用）。
- 将来の Fyne GUI 化は `internal/presentation/gui/` を追加し、`cmd/goscraper-gui/main.go` を別途用意することで、CLI 版とユースケース層以下を完全に共有できる。

### レイヤー責務と依存ルール

| レイヤー | 責務 | 依存可能な相手 |
| --- | --- | --- |
| `presentation` | 入出力の窓口。CLI のフラグ解析、GUI のイベントを `usecase` 呼び出しに変換する。 | `usecase`, `domain/model` |
...
| `infrastructure` | HTTP・ファイル・robots.txt 等の外部I/O実装。 | `domain` |
| `plugins/*` | 具体プラグイン実装。`init()` で `core` のレジストリへ登録。 | `domain`, `core`（登録のみ） |
...
- `presentation` は `usecase` と `domain/model` のみに依存する。
- `plugins/*` は `domain` 抽象を実装し、`core` のレジストリ登録APIにだけ依存する（カーネル本体や他プラグインを直接 import しない）。
- `cmd/` だけが具体プラグインを `_` import する。

### プレゼンテーション層の方針

- 当面の動作確認は `cmd/goscraper/main.go` から CLI（`internal/presentation/cli`）を呼び出す形にする。
- CLI と GUI の双方は **同じ `usecase` インターフェース** を呼ぶ。`usecase` は `presentation` 種別を意識しない。
- 進捗通知やイベントは GUI 化を見越して **コールバック or chan ベース** で `usecase` から渡せるようにする（同期戻り値だけにしない）。
- Fyne への移行手順の想定:
    1. `internal/presentation/gui/` 配下に Fyne UI を実装。
    2. `cmd/goscraper-gui/main.go` を追加し、CLI と同じ usecase を bind する。
    3. CLI 版バイナリは引き続き `cmd/goscraper` から生成。
...
flowchart TD
    ...
    Start --> UC --> K --> Pipeline
    Pipeline --> P2 --> HTTP --> P5 --> P7 --> P6 --> Out
    P7 --> P8 --> Queue
    Queue -. next URL .-> Pipeline
```

- P2〜P8 のうち、HTTP 取得とキュー/出力はカーネル固定実装。それ以外はプラグインで差し替え可能。
...
- 起動順:
    ...
    2. `Config` 構築・バリデーション
    3. カーネル生成（`Host` を初期化してプラグインに渡す準備）
    4. レジストリにある全プラグインに対して `Init(ctx, host)` 呼び出し
    5. ユースケース実行（パイプライン or クローラ起動）
    6. 終了シグナル受信時、プラグインを **登録の逆順** で `Close(ctx)`
- 失敗ポリシー:
...
- `cmd/goscraper/main.go` のみが具体プラグインを副作用 import している。
- CLI と将来の GUI が **同一の usecase シグネチャ** を共有する。
- ディレクトリ構成は上記のとおりであり、レイヤー越境の依存はビルド時 import ツリー（`go list -deps`）で確認可能。
