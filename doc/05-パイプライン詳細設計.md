...
flowchart TD
    ...
    Robots[Check robots.txt cache]
    Worker[Worker N goroutine]
    Pipeline[Pipeline run]
    P2[P2 PreProcessor chain]
    HTTP[HTTP Get with retry and timeout]
    ...
    Start --> Frontier --> Robots --> Worker --> Pipeline
    Pipeline --> P2 --> HTTP --> Detect
    Detect -->|HTML| P5H
    Detect -->|PDF| P5P
    P5H --> P7 --> P6 --> Out
    P5P --> P7 --> P6 --> Out
...
- 動作: `infrastructure/httpclient` を使い `GET`。
- リトライ:
    - `RequestConfig.RetryCount` 回まで再試行。
    - 対象は `5xx` ステータス、ネットワークエラー、タイムアウト。
    - `RequestConfig.RetryInterval` 待機後に再試行（指数バックオフは今回採用しない）。
- タイムアウト:
    - `context.WithTimeout(parent, RequestConfig.Timeout)` をリクエストに付与。
...
type Crawler struct {
    ...
    robots   *RobotsCache // robots.txt をホスト単位でキャッシュ
}
...
- 取得失敗時の挙動:
    - 404: 全許可とみなす。
    - その他のエラー: 全許可とみなす（保守的にはせず利用性優先）。警告ログを残す。
- 評価対象 User-Agent: `request.headers["User-Agent"]`（未設定なら `*`）。
- 不許可と判定された URL は `seen` には登録しつつ、ワーカーには渡さず破棄。
