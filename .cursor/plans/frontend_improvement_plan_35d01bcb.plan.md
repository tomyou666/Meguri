...
| 一括スクレイプの実行仕様なし | Stage 4 に **RunMode 2・選択ノード必須** を明記 |
...
| R29 | 複数選択の一括スクレイピング | 4 | **RunMode 2**、選択ノードを順にキュー（既存スタブ拡張） |
...
- **3 種差分**（`getWorkspaceDiff` 戻り値）:
  1. **content** — `node_results` 本文ハッシュ / 要約テキスト差
  2. **links** — 出辺集合の増減（分岐差）
  3. **fetch** — success / error / 未取得（skipped 含む）の変化
...
7. **一括スクレイプ = RunMode 2**、結果保存 = `saveResults` を明記
