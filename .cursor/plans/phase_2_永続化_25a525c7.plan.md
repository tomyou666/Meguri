...
```powershell
migrate -path internal/app/migrations -database "sqlite://data/scraperbot.db" up
migrate -path internal/app/migrations -database "sqlite://data/scraperbot.db" version
```
...
| 変数 | 既定値 | 説明 |
|------|--------|------|
| `DB` | `data/scraperbot.db` | 開発用 SQLite パス（`.gitignore`） |
...
| 生成される DB ファイル | `data/scraperbot.db`（dev、`dbpath_dev`）— **git 管理外** |
