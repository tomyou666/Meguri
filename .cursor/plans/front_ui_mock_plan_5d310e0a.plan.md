...
| クロール進捗の配信（Phase 3+） | **Wails 組み込み Event**（pub/sub）。Go が `Events.Emit`、TS が `Events.On` で購読。WebSocket / SSE は使わない |
...
Phase 3（後日）では `CrawlStub` を Wails Service + `backend` in-process + **Wails 組み込み Event（pub/sub）** に差し替え（詳細は下記「クロール進捗: Wails Events」）。

---

## クロール進捗: Wails Events（Phase 3 確定）

デスクトップ（Wails）では、クロール中のノード単位更新を **HTTP ストリーム（SSE / WebSocket）ではなく、Wails の組み込み Event** で配信する。

| 層 | 役割 |
|----|------|
| Go `ScraperService` | `StartCrawl` RPC でジョブ開始。`backend` crawler の `ResultSink` / 将来の `ProgressEvent` から進捗を受け取り、`application.Events.Emit(ctx, topic, payload)` で発火 |
| Wails runtime | トピック名による **pub/sub**（複数購読可・プロセス内） |
| TS `WailsScraperAdapter` | `Events.On(topic, handler)` で購読し、既存 `StartCrawlParams` のコールバック（`onNodeStarted` 等）へ橋渡し |
| `appStore.startCrawl` | **変更なし** — 引き続き `ScraperPort.startCrawl` のコールバックで Zustand を更新 |

**トピック例**（`docs/api/scraper-ui.md` と同期）:

- `scraper:crawl:nodeStarted`
- `scraper:crawl:nodeSucceeded`
- `scraper:crawl:nodeFailed`
- `scraper:crawl:nodeSkipped`
- `scraper:crawl:edgeDiscovered`
- `scraper:crawl:completed`
- `scraper:crawl:error`

payload は `workspaceId` + イベント固有フィールド（`StartCrawlParams` のコールバック引数と同型）。

**Phase 1（現状）**: `MockScraperAdapter` が `crawlStub` 内で同期的にコールバック呼び出し（Wails 未使用）。

**HTTP REST**（将来のリモート API）: `StartCrawl` / `PauseCrawl` / `StopCrawl` はコマンド POST のみ。進捗の SSE 等は **Wails デスクトップの正規経路とは別問題**として必要になった時点で別途設計する。

```mermaid
sequenceDiagram
  participant Store as appStore
  participant Adapter as WailsScraperAdapter
  participant Go as ScraperService
  participant Crawler as backend Crawler
  Store->>Adapter: startCrawl(callbacks)
  Adapter->>Go: StartCrawl RPC
  Adapter->>Adapter: Events.On(topics)
  loop each page
    Crawler-->>Go: ResultSink / ProgressEvent
    Go-->>Adapter: Events.Emit(topic, payload)
    Adapter-->>Store: onNodeStarted / onNodeSucceeded ...
  end
  Go-->>Adapter: Events.Emit(completed)
...
- **クロール進捗は Wails 組み込み Event（pub/sub）のみ** — `Events.Emit`（Go）/ `Events.On`（TS）。WebSocket・SSE は採用しない
- `ResultSink` / 将来の `ProgressEvent` → 上記 Event トピックへ変換して `Emit`
- `StartCrawl` / `PauseCrawl` / `StopCrawl` は Wails メソッド（RPC）。進捗本体は RPC 戻り値に載せない
- context による pause/stop
- `WailsScraperAdapter` で Event 購読 → `StartCrawlParams` コールバックへ橋渡し（`appStore` の購読 API は維持）
- `MockScraperAdapter` / `crawlStub` は開発・テスト用に残すか feature flag で切替

### Phase 4 — UI 改善（frontend_improvement_plan 実装済み）
- **DDL 契約**: [`front/storage/schema.sql`](front/storage/schema.sql)（ランタイム SQLite は Phase 2）
- **Adapter**: `ScraperPort` + `MockScraperAdapter`（[`docs/api/scraper-ui.md`](docs/api/scraper-ui.md)）
- **上書き決定**: ショートカット（Ctrl+Z/Y/C/V/A）、zundo、手動エッジ追加、JSON 設定エディタ廃止→フォームのみ
- **エッジ制約**: 同一 source→target 1 本（複数 outgoing 可）
- **差分 UI**: `getWorkspaceDiff` — content / links / fetch。左 SB の差分アイコン、ノード Badge
  - **links 差分**: `node_results.links_json` のみ（グラフ出辺は使わない）
  - **baseline**: `workspaces.baseline_run_id` → `crawl_runs` + 当該 run の `node_results`
- **結果**: 永続層は adapter 経由（`getNodeResult` / `mergeResults` 等）。失敗行も `node_results.error` に保存
- **履歴**: ノードごと最大 20 行。削除は最新 1 件のみ

---

## フロント改善で上書きした Phase 1 決定

| 旧 Phase 1 | 改善後 |
|------------|--------|
| ショートカットなし | Ctrl+Z/Y/C/V/A、矩形選択、Shift 連続選択 |
| undo なし | zundo（編集のみ） |
| 手動エッジ追加不可 | onConnect + 削除 |
| MenuBar JSON 設定 | フォーム（request/content/pdf/crawl、app は +plugins/output） |
| crawlStub 直結 | ScraperPort → Mock（内部 crawlStub） |
| lastResult を store のみ | adapter 永続層 + hydrate |
