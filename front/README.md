...
| `internal/infrastructure/` | リポジトリ、`.crawlproj` / `.scrb`、dialog |
...
| パス | 内容 |
|------|------|
| `frontend/bindings/meguri-app/index.ts` | サービス export 集約 |
| `frontend/bindings/meguri-app/internal/usecase/wails_service/storeservice.ts` | `StoreService` RPC |
| `frontend/bindings/meguri-app/internal/usecase/wails_service/projectservice.ts` | `ProjectService` RPC |
| `frontend/bindings/meguri-app/internal/usecase/wails_service/index.ts` | 上記 re-export |
| `frontend/bindings/meguri-app/internal/model/models.ts` | Go DTO の TS 型 |
| `frontend/bindings/meguri-app/internal/model/index.ts` | model re-export |
...
| 前提 | `make migrate-up` 済みの `data/meguri.db` |
...
| 種別 | コマンド | 出力先 | 手編集 |
|------|----------|--------|--------|
| DB スキーマ | `make migrate-up` / 起動時 `Up` | `data/meguri.db` + `schema_migrations` | — |
